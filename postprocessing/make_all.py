...
def evaluation_row() -> str:
    ...
    return (
        "**How frames are drawn.** One sample is a within-chunk clip at the target frame rate; "
        "`_plan_match` enumerates every clip of every match, with no per-match cap and no "
        f"reweighting. The training index holds {sm['matches']} matches.\n\n"
