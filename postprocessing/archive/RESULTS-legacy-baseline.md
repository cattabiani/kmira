# ARCHIVED 2026-09-11 — every absolute number here rests on the retired baseline

**Do not quote anything on this page.** It is kept for the qualitative findings and the method,
both of which survive; the numbers do not.

The baseline run this page is built on trained its first seven chunks on a fixed `run.seed=28` —
the data-repetition bug — so steps 0–56,000 replayed the same **53.4%** of the training data seven
times and **46.6% was never seen**. Its 272,000-step elbow, its 24.747 dB plateau and its training
speed are artifacts of that. A clean replacement (`baseline_v2`) was **+1.87 dB ahead by step
56,000**.

What survives, and is worth reading this page for:

- **Section 0c** — per-chunk data slices are wildly uneven, and both "false plateaus" were the seed
  schedule. Corroborated in the warm-start arms, which never replayed, so it is independent of
  the bug.
- **Experiments 1 and 2** are **paired** comparisons whose arms all share the retired origin and
  seed schedule, so the *arm-to-arm* findings (learned mixing beats its control; the gain is mostly
  reach, not freedom; each arm goes as shallow as it is allowed; P-DINO does not separate the arms)
  stand as directions. Their **magnitudes do not**, because the baseline they sit on was
  undertrained for its first 56,000 steps.
- **The method** — matched steps, paired seed schedules, generated-not-typed numbers.

Current results: [`../RESULTS.md`](../RESULTS.md). The rebuild plan is in
[`../../AGENTS.md`](../../AGENTS.md).
